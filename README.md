...
- ggplot2 treats the horizontal axis as continuous; the default should be for it to be treated as categorical, with placement determined by axis aesthetics (in the multivariate setting) or by `as(x, "numeric")` (in the bivariate setting), and labeled accordingly. (All categorical variables should be stored as factors anyway.) The present implementation makes this challenging, but an easy fix is to add a `scale_x_continuous()` layer (see `help(geom_alluvium)`).
