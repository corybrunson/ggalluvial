...
#' \code{is_alluvial_old} will default to \code{is_alluvial_alluvia_old} and assume that
#' all fields in \code{data} (other than \code{weight}, if given) are to be
#' treated as axes.
#' 

#' \code{to_lodes_old} takes a data frame with several designated variables to be 
...
#' 
#' \code{to_alluvia_old} takes a data frame with axis and axis value variables to be
...
#' @export
is_alluvial_old <- function(data, ..., logical = TRUE, silent = FALSE) {
  ...
  if (!is.null(dots$key) | !is.null(dots$value) | !is.null(dots$id)) {
    ...
    is_alluvial_lodes_old(data = data, ..., logical = logical, silent = silent)
  } else {
    is_alluvial_alluvia_old(data = data, ..., logical = logical, silent = silent)
  }
}

#' @rdname alluvial-data
#' @export
is_alluvial_lodes_old <- function(data,
                                  key, value, id,
                                  weight = NULL,
...
#' @rdname alluvial-data
#' @export
is_alluvial_alluvia_old <- function(data,
                                    axes = NULL,
                                    weight = NULL,
                                    logical = TRUE, silent = FALSE) {
...
#' @rdname alluvial-data
#' @export
to_lodes_old <- function(data,
                         key = "x", value = "stratum", id = "alluvium",
                         axes, diffuse = FALSE, discern = FALSE) {
  
  stopifnot(is_alluvial_old(data, axes = axes, silent = TRUE))
...
#' @rdname alluvial-data
#' @export
to_alluvia_old <- function(data, key, value, id,
                           distill = FALSE) {
  ...
  stopifnot(is_alluvial_old(data, key = key, value = value, id = id, silent = TRUE))
