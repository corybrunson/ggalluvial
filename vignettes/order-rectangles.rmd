...
Additionally, the reordering of cases within each collection relies on a hierarchy of sorting variables, based on the strata at nearby axes as well as the present one and, optionally, on the values of differentiation aesthetics like `fill`, and the behavior depends on the choices of several plotting parameters (`decreasing`, `reverse`, and `absolute`).
Only once the coordinates are computed do the geom layers use them to position the rectangles and splines that constitute the plot.
Thus, the results of the statistical transformation are not as intrinsically meaningful as others, but serve to underpin a more interpretable visualization.
...
(There's no good way at this point to nudge these labels toward the flow to which they correspond, but that's a potential future feature.)
...
The `aes.bind` parameter defaults to `"none"`, in which case aesthetics play no role in the order of the lodes. Setting the parameter to `"linked"` prioritizes any such aesthetics _after_ the strata of any other axes but _before_ the alluvia of the index axis, while setting it to `"index"` prioritizes aesthetics _before_ the strata of any other axes.
...
ggplot(toy, aes(x = collection, stratum = category, alluvium = subject)) +
  stat_alluvium(aes(fill = class), aes.bind = "index") +
  stat_stratum() +
...
ggplot(toy, aes(x = collection, stratum = category, alluvium = subject)) +
  stat_alluvium(aes(fill = class), aes.bind = "linked") +
  stat_stratum() +
  stat_alluvium(geom = "text", aes(fill = class, label = subject),
                aes.bind = "linked")
```
...
Setting `aes.bind` to a non-default option tells `stat_flow()` to prioritize flow aesthetics after the index axis, before the alluvium variable, and either after (`"linked"`) or before (`"index"`) the strata of the adjacent axis:

```{r flows plots w/ aesthetic binding}
ggplot(toy, aes(x = collection, stratum = category, alluvium = subject)) +
  stat_flow(aes(fill = class), aes.bind = "linked") +
  stat_stratum() +
  stat_flow(geom = "text", aes(fill = class, label = subject),
            aes.bind = "linked")
ggplot(toy, aes(x = collection, stratum = category, alluvium = subject)) +
  stat_flow(aes(fill = class), aes.bind = "index") +
  stat_stratum() +
  stat_flow(geom = "text", aes(fill = class, label = subject),
            aes.bind = "index")
```
...
Finally, one may wish to put the lodes at each axis in a predefined order, subject to their being located in the correct strata. This can be done by passing an integer matrix or a list of integer vectors to `lode.ordering`, which takes the columns or vectors to prescribe the order of the cases at the axes.
(Internally, lode guidance functions are used to produce a lode ordering matrix that is then used in the same way.)
