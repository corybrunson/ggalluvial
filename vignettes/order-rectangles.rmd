...
Setting `aes.bind` to a non-default option tells `stat_flow()` to prioritize flow aesthetics after the index axis, before the alluvium variable, and either after (`"flows"`) or before (`"alluvia"`) the strata of the adjacent axis:
...
ggplot(toy, aes(x = collection, stratum = category, alluvium = subject)) +
  ...
  stat_flow(geom = "text", aes(fill = class, label = subject),
            aes.bind = "flows")
ggplot(toy, aes(x = collection, stratum = category, alluvium = subject)) +
  stat_flow(aes(fill = class), aes.bind = "alluvia") +
  stat_stratum() +
  stat_flow(geom = "text", aes(fill = class, label = subject),
            aes.bind = "alluvia")
```
