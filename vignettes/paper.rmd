...
Alluvial plots use variable-width ribbons and stacked bar plots to represent multi-dimensional or repeated-measures data with categorical or ordinal variables [@Rosvall2010;@Bojanowski2016]. This package extends the layered grammar of graphics [@Wickham2016] to generate alluvial plots from tidy data [@Wickham2014].
...
The following alluvial plot depicts several meaningful stat--geom combinations that appear in the documentation:

```{r alluvial stats and geoms, echo=FALSE, message=FALSE, warning=FALSE}
library(ggalluvial)
combos <- data.frame(
  stat = c(
    rep("stratum", 2),
...
alluvials <- c("stratum", "alluvium", "flow", "lode")
ggplot(transform(combos,
                 both_alluvial = stat %in% alluvials & geom %in% alluvials),
       aes(axis1 = stat, axis2 = geom)) +
  geom_alluvium(aes(fill = both_alluvial)) +
  geom_stratum() +
...
In particular, parallel sets plots, adapted from parallel coordinates plots [@Inselberg1987;@Wegman1990] to categorical data, represent cohorts of equivalent cases as parallelograms connecting sets represented as rectangles [@Hofmann2013].
