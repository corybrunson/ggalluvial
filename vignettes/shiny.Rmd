...
The UC-Berkeley admissions dataset is used in this example. After loading the necessary packages, the first thing we do in the app is load the data.
...
Next we set `offset`, the distance from cursor to tooltip, in pixels, in both x and y directions. We also set `node_width` here which is used as an argument to `geom_stratum()` below, and is used again later to determine whether the mouse cursor is hovering over a stratum.
...
The coordinates we have are in grid plotting units but we need to convert them into the same units as the axes on the plot. We do this by determining the range of the x and y axes in grid units (`xrange_old` and `yrange_old`). Then we fix the range of the x axis as 1 to the number of strata, adjusted by the width of the nodes (the value of `1/6` is internal to **ggalluvial**). Next we fix the range of the y axis to the sum of the counts across all alluvia at one node.
...
We get the name of the stratum as well as its width from the data in `pbuilt`.
...
We render a tooltip using very similar syntax to the one above.
