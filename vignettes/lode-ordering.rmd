...
Four alternative `lode_*()` functions are available: `"rightleft"` and `"leftright"`, which behave like `"zigzag"` but extend completely in one outward direction from axis $i$ before the other; and `"rightward"` and `"leftward"`, which put the remaining axes in increasing and decreasing order. Two are illustrated below:

```{r alluvia plot w/ leftright guidance}
for (i in 1:4) print(lode_leftright(4, i))
ggplot(toy, aes(x = collection, stratum = category, alluvium = subject)) +
  stat_alluvium(aes(fill = class), lode.guidance = "leftright") +
  stat_stratum() +
  stat_stratum(geom = "text", aes(label = category))
```

The difference between `"leftright"` guidance and `"zigzag"` guidance can be seen in the order of the lodes of the `"Y"` stratum at axis `3`: Whereas `"zigzag"` minimized the crossings between axes `3` and `4`, locating the distinctive class-`"one"` case above the others, `"leftright"` minimized the crossings between axes `2` and `3` (axis `2` being immediately leftward of axis `3`), locating this case below the others.

```{r alluvia plot w/ leftward guidance}
for (i in 1:4) print(lode_leftward(4, i))
ggplot(toy, aes(x = collection, stratum = category, alluvium = subject)) +
  stat_alluvium(aes(fill = class), lode.guidance = "leftward") +
  stat_stratum() +
  stat_stratum(geom = "text", aes(label = category))
```

The effect of `"leftward"` guidance is to keep the right part of the diagram as tidy as possible while allowing the left part to become as messy as necessary. (`"rightward"` has the opposite effect.)
...
For the toy example, we can put the cases in the order of their IDs in the data:
...
Within each stratum at each axis, the cases are now in order from bottom to top. A shortcut is in the works to allow to pass a single numerical vector to `lode.ordering`, having length the number of cases, which will be used to sort the lodes at each axis.
