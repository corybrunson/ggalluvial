...
The format allows us to assign aesthetics that change from axis to axis along the same alluvium, which is useful for repeated measures datasets. It also gives us the option to aggregate the flows between adjacent axes.
We can demonstrate these options on data from the influenza vaccination surveys conducted by the [RAND American Life Panel](https://alpdata.rand.org/):
