...
N.B.: The app demonstrated here depends on the packages `htmltools` and `sp`, in addition to the obvious `ggalluvial` and `shiny`. Please be aware that all those packages will need to be installed on the server where your Shiny app is running.
...
The first snippet includes code to draw the plot and build it with `ggplot_build()`. Note that the global assignment operator `<<-` is used to assign `node_width` and `pbuilt` so they are both accessible outside the `renderPlot()` expression.
...
```{r server part 1b, eval = FALSE}
  ...
  xspline_coords <- lapply(group_xsplines, function(coords) grid::xsplineGrob(x=coords$x, y=coords$y, shape=coords$shape, open=FALSE))
...
We define a function `new_range_transform()` inline and apply it to each set of coordinates. Now we have the coordinates of the polygons in plot units! So we can close the expression after returning the plot.
...
Finally, we render a tooltip using the `div` tag and passing it to `htmltools::renderTags()`. Note that the tooltip positioning is provided in CSS coordinates, not data coordinates. This does not require any additional effort on our part because the `hover` information also returns the mouse cursor location in those units.
...
If at least one polygon is beneath the mouse cursor, we locate the corresponding row in the input data and extract information to display in the tooltip. In the situation where there are more than one polygon overlapping, we get the information for the polygon that is plotted last, meaning it will appear "on top" in the plot. In this example, we will display the names of all the nodes that the alluvium passes through.
