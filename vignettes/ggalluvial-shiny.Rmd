...
Now for the hard part: reverse-engineering the locations of the alluvia polygons. This makes use of `pbuilt$data[[1]]`, a dataframe with the individual elements of the alluvial plot. We add an additional column for `width`, which has a value of `1/3` hard-coded into `ggalluvial::geom_alluvium()`. We defined the `draw_by_group()` function above, which is slightly modified from `ggalluvial::geom_alluvium()` and uses the unexported function `ggalluvial:::data_to_xspline()`. The function is applied to each polygon to get the xspline coordinates. Then, we feed the xspline coordinates into the function `grid::xsplineGrob()` and the resulting grob into `grid::xsplinePoints()`. At this point we now have the coordinates of the alluvia polygons.
...
We apply the function `new_range_transform()` we defined above to each set of coordinates. Now we have the coordinates of the polygons in plot units! So we can close the expression after returning the plot.
...
Finally, we render a tooltip using the `div` tag. Note that the tooltip positioning is provided in CSS coordinates, not data coordinates. This does not require any additional effort on our part because the `hover` information also returns the mouse cursor location in those units.
...
# Define function to draw xspline (modified from ggalluvial)
